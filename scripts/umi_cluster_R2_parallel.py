...
def main():
    ...
    ### Run UMI clustering in parallel
    strain_data = [(strain, df_gpd.loc[df_gpd['strain'] == strain].copy()) for strain in df_gpd['strain'].unique()]
    # Parallel processing
    with Pool(processes=args.threads) as pool:  # threads passed as argument
        results = pool.map(process_single_strain, strain_data)
