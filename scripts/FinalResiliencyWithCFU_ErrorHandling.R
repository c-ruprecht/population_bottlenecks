getNrNb <- function(ReadsTable, CFUtable, InocCFU, WhereAreReferences, minweight, CorrectForNoise, outputfilename, CalibrationFile = NULL) {
 ...
 #Makes a table for estimating bottleneck from fraction of barcodes that are identified
    ...
    result <- tryCatch({
      ...
      scanformin <- function(p) {
        ...
        findmin <- function() {
          ...
          # Add validation checks for newlocation
          if(is.na(newlocation) || is.infinite(newlocation) || length(na.omit(x)) == 0) {
            return(start)
          }
          if(newlocation > length(na.omit(x))) {newlocation <- where}
          if(newlocation < 1) {newlocation <- where}
          newstart <- x[round(newlocation)]
          if(!is.na(newstart) && newstart < start) {start <- newstart}
          start<<-start
        }
      ...
      # If an error occurs, return NA values with the error message
  ...
  # Add column names including the error description column
  if(!is.null(CalibrationFile)) {
    colnames(TableOfEstimates) <- c("Nb", "Nr", "NbLower_Cal", "NbMedian_Cal", "NbUpper_Cal", "NrLower_Cal", "NrMedian_Cal", "NrUpper_Cal", "Error_Description")
  } else {
